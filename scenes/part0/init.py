...
class InitScene(Scene):
    ...
    def construct(self):
        ...
        self.wait(max(0.0, (voice_1_duration or 0.0) - 4.5))
        ...
        for num, label in parts_data:
            # Tạo vòng tròn bao quanh mỗi số thứ tự
            circle = Circle(radius=0.35, stroke_width=2.5, color=VG_BLUE)
            num_text = VGText(
                num, font_size=24, color=VG_BLUE, weight=BOLD_WEIGHT
            ).move_to(circle.get_center())
            badge = VGroup(circle, num_text)

            label_text = VGText(label, font_size=28, color=WHITE)
            # Đặt nội dung chữ bên phải số thứ tự
            label_text.next_to(badge, RIGHT, buff=0.5)

            row = VGroup(badge, label_text)
            part_items.add(row)

        part_items.arrange(DOWN, buff=0.5, aligned_edge=LEFT)
        part_items.next_to(overview_line, DOWN, buff=0.5)

        item_anim_time = 0.6
        base_intro_wait = 2.5
        base_item_waits = [6.9, 4.9, 5.9, 7.9, 5.14]
        ...
        self.wait(max(0.0, (voice_2_duration or 0.0) - 2.5))
